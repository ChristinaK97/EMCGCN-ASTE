...
class RefiningStrategy(nn.Module):
    def __init__(self, hidden_dim, edge_dim, dim_e, dropout_ratio=0.5, use_refining=True):
...
class GraphConvLayer(nn.Module):
    ...
    def __init__(self, device, gcn_dim, edge_dim, dep_embed_dim, pooling='avg', use_refining=True):
        ...
        self.highway = RefiningStrategy(gcn_dim, self.edge_dim, self.dep_embed_dim, dropout_ratio=0.5, use_refining=use_refining)
...
class EMCGCN(torch.nn.Module):
    def __init__(self, args, LFtoUse):
        super(EMCGCN, self).__init__()
        self.args = args
        self.LFtoUse = LFtoUse
        ...
        self.feat_emb = {
            feature: torch.nn.Embedding(getattr(args, f'{feature}_size'), args.class_num, padding_idx=0,
                                        device=args.device)
        ...
        for i in range(self.num_layers):
            self.gcn_layers.append(
                GraphConvLayer(args.device, args.gcn_dim,
                               (1+len(self.LFtoUse))*args.class_num, args.class_num, args.pooling,
                               use_refining=self.args.use_refining)
            )
    ...
    def forward(self, tokens, masks, word_pair_feature):
        ...
        self_loop = torch.stack(self_loop).to(self.args.device).unsqueeze(1) \
                        .expand(batch, (1 + len(self.LFtoUse)) * self.args.class_num, seq, seq) * tensor_masks.permute(0, 3, 1, 2) \
                        .contiguous()
